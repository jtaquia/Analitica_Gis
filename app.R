...
ui <- fluidPage( 
  ...
  sidebarLayout(position = "left",
                ...
                sidebarPanel(
                  sliderInput("bins","Number of Repartos:",min = 10,max = 25, value = 20)
                  
                  ,hr(),
                  sliderInput("C_over","Margen:",min = 0.25,max = 1, value = 0.5)
...
server <- function(input, output, session) { 
  ...
  datasetInput <- reactive({  
    ...
    output$newmap <- renderLeaflet({data3 %>% 
        arrange(id_order) %>% 
        leaflet() %>% 
        addTiles() %>% 
        addCircleMarkers(
